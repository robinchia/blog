...
保存这个session id的方式可以采用cookie，这样在交互过程中浏览器可以自动的按照规则把这个标识发挥给服务器。一般这个cookie的名字都是类似于SEEESIONID，而。比如weblogic对于web应用程序生成的cookie，JSESSIONID= ByOK3vjFD75aPnrF7C2HmdnV6QZcEbzWoWiBYEnLerjQ99zWpBng!-145788764，它的名字就是 JSESSIONID。由于cookie可以被人为的禁止，必须有其他机制以便在cookie被禁止时仍然能够把session id传递回服务器。经常被使用的一种技术叫做URL重写，就是把session id直接附加在URL路径的后面，附加方式也有两种，一种是作为URL路径的附加信息，表现形式为http://...../xxx;jsessionid= ByOK3vjFD75aPnrF7C2HmdnV6QZcEbzWoWiBYEnLerjQ99zWpBng!-145788764
...
**[html]** [view plain](http://blog.csdn.net/# "view plain")[copy](http://blog.csdn.net/# "copy")[print](http://blog.csdn.net/# "print")[?](http://blog.csdn.net/# "?")

1. < % cookie[]cookies = request.getCookies();  
1. if (cookies.lenght == 0 || cookies == null)  
1. doStuffForNewbie();  
1. //没有访问过   
1. }else  
1. {  
1. doStuffForReturnVisitor(); //已经访问过了  
1. }% >  
...
1. cookie[]cookies = request.getCookies();  
1. if (cookies.lenght == 0 || cookies == null)  
1. out.println("Has not visited this website");  
1. }else{  
1. for (int i = 0; i < cookie.length; i++){  
1. out.println("cookie name:" + cookies[i].getName() + "cookie value:" +  
1. cookie[i].getValue());}}  
...
推荐：[Java程序员从笨鸟到菜鸟之（二十四）Xml基础详解和DTD验证](http://blog.csdn.net/csh624366188/article/details/7428222)
来源： <[http://blog.csdn.net/csh624366188/article/details/7425770](http://blog.csdn.net/csh624366188/article/details/7425770)> 
